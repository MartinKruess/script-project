...
def render_hotkeys_table(frame, modis):
    headers = ["Ziel", "Typ", "Hotkey", "Aktion"]
    ...
    for i, hk in enumerate(hotkeys, start=1):
        ...
        ctk.CTkLabel(frame, text=target).grid(row=i, column=0, padx=5, pady=2)

        ctk.CTkLabel(frame, text=hk["type"]).grid(row=i, column=1, padx=5, pady=2)
        ctk.CTkLabel(frame, text=hk["hotkey"]).grid(row=i, column=2, padx=5, pady=2)
